  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-navy text-white pt-20">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center z-10 flex flex-col items-center">
        <div className="inline-block mb-6 px-4 py-1 border border-gold/50 rounded-full bg-navy-light/30 backdrop-blur-sm">
        ...
        <h1 className="font-serif text-4xl md:text-6xl lg:text-7xl font-medium leading-tight mb-8">
          <span className="block text-white/90">The Foundation of</span>
          <span className="block text-gold italic">Luxury Authority</span>
        </h1>
        
        <div className="max-w-4xl mx-auto bg-white/5 backdrop-blur-sm border border-white/10 p-6 md:p-8 rounded-lg mb-12 transform hover:scale-[1.01] transition-transform duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-5xl mb-12">
          {DOMAIN_STATS.map((stat, index) => (
            <div key={index} className="flex flex-col items-center p-6 bg-navy-light/20 border border-white/5 rounded-md hover:border-gold/30 transition-colors">
              ...
              <span className="text-white/60 text-xs">{stat.subtext}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
          ...
          <a href="#value-proposition" className="px-8 py-4 bg-transparent border border-white/30 text-white font-semibold rounded-sm hover:bg-white/5 transition-all text-center min-w-[200px]">
            Why This Asset?
          </a>
        </div>
      </div>
